...
public class BossHealth : MonoBehaviour, Interfaces.IDamage, Interfaces.IDamageSpecial
{
     ...
     public void TakeDamageSpecial(int damage, Vector3 hitPosition, Transform textRotateTarget, string special, int percentage)
     {
          Color textColor = Color.white;
          health -= damage; FloatingDamage(damage, hitPosition, textRotateTarget, textColor); //instancia o dano normal
          
          switch (special)
          {
               case "MaxHealth":
                    textColor = Color.green;
                    damage = Mathf.FloorToInt(maxHealth * percentage/500f);
                    break;
               
               case "MissingHealth":
                    textColor = Color.black;
                    damage = Mathf.FloorToInt((maxHealth - health) * percentage/500f);
                    break;
               
               case "CurrentHealth":
                    textColor = Color.yellow;
                    damage = Mathf.FloorToInt(health * percentage/500f);
                    break;

               case "Extra":
                    textColor = Color.grey;
                    damage = Mathf.FloorToInt(damage * percentage/500f);
                    break;
          }
